                    ...
                    <div class="card shadow-lg border-0 rounded-4">
                        <div class="card-body p-4 p-md-4 p-lg-4">
                            <div class="text-center mb-4">
                                <h1 class="login-title mb-2">QUÊN MẬT KHẨU</h1>
                                <p class="text-muted mb-0">
                                    Nhớ mật khẩu?
                                    <router-link to="/client/dang-nhap" class="register-link text-decoration-none fw-semibold">
                                        Đăng nhập ngay
                                    </router-link>
                                </p>
                            </div>
                            ...
                            <form @submit.prevent="handleForgotPassword" class="login-form">
                                ...
                                <div class="text-center">
                                    <router-link to="/client/dang-nhap" class="forgot-password-link text-decoration-none">
...
export default {
    ...
    methods: {
        async handleForgotPassword() {
            ...
            try {
                // TODO: Tạo API quên mật khẩu trong backend
                // Endpoint: /api/khach-hang/quen-mat-khau
                const response = await api.post('/khach-hang/quen-mat-khau', {
                    email: this.formData.email,
                });

                if (response.data.success) {
                    this.success = response.data.message || 'Chúng tôi đã gửi hướng dẫn đặt lại mật khẩu đến email của bạn. Vui lòng kiểm tra hộp thư!';
                    
                    // Reset form
                    this.formData.email = '';
                } else {
                    this.error = response.data.message || 'Có lỗi xảy ra. Vui lòng thử lại!';
                }
            } catch (error) {
                console.error('Forgot password error:', error);
                if (error.response?.status === 404) {
                    this.error = 'Email hoặc tài khoản không tồn tại trong hệ thống!';
                    ...
                    // Nếu API chưa tồn tại, hiển thị thông báo tạm thời
                    this.error = 'Tính năng quên mật khẩu đang được phát triển. Vui lòng liên hệ admin để được hỗ trợ!';
                }
            } finally {
                this.loading = false;
            }
